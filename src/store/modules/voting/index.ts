...
import { GetVotingInput, GetLastVotingInput } from "@/services/api/types";
...
const getters: GetterTree<VotingState, RootState> = {
  ...
  stats(state: VotingState) {
    return {
      ...
      count: state.voting!.votes!.totalCount || 0,
    };
  },
...
const actions: ActionTree<VotingState, RootState> = {
  ...
  async getLastVoting(
    {commit, state}: {commit: Commit, state: VotingState},
    input: GetLastVotingInput,
  ) {
    commit("getVotingRequest");
    try {
      const voting = await api.votings.getLastVoting(input);
      commit("getVotingSuccess", voting);
      return voting;
    } catch (error) {
      commit("getVotingError", error.code);
      return;
    }
  },
};
